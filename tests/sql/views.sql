...
SELECT name IS NOT NULL as name_not_null,moorage,moorage_id,date_trunc('minute', duration::INTERVAL) AS duration,stayed_at,stayed_at_id,arrived IS NOT NULL AS arrived_not_null,arrived_log_id,arrived_from_moorage_id,arrived_from_moorage_name,departed IS NOT NULL AS departed_not_null,departed_log_id,departed_to_moorage_id,departed_to_moorage_name,notes FROM api.stays_view ORDER BY name ASC;

\echo 'stay view'
--SELECT * FROM api.stay_view;
SELECT name IS NOT NULL as name_not_null,moorage,moorage_id,date_trunc('minute', duration::INTERVAL) AS duration,stayed_at,stayed_at_id,arrived IS NOT NULL AS arrived_not_null,arrived_log_id,arrived_from_moorage_id,arrived_from_moorage_name,departed IS NOT NULL AS departed_not_null,departed_log_id,departed_to_moorage_id,departed_to_moorage_name,notes FROM api.stay_view ORDER BY name ASC;

\echo 'moorages view'
--SELECT * FROM api.moorages_view;
SELECT moorage,default_stay,default_stay_id,arrivals_departures,date_trunc('minute', total_duration::INTERVAL) AS total_duration FROM api.moorages_view ORDER BY moorage ASC;

\echo 'moorage view'
--SELECT * FROM api.moorage_view;
SELECT name,default_stay,default_stay_id,date_trunc('minute', stays_sum_duration::INTERVAL) AS stays_sum_duration,notes,logs_count,stays_count,home,geog,stay_first_seen IS NOT NULL AS stay_first_seen_not_null, stay_last_seen IS NOT NULL AS stay_last_seen_not_null, stay_first_seen_id, stay_last_seen_id FROM api.moorage_view ORDER BY name ASC;

\echo 'logs geojson view'
--SELECT * FROM api.logs_geojson_view;
SELECT id,name,geojson->'geometry' IS NOT NULL AS geojson_geometry_not_null, geojson->'properties' IS NOT NULL AS geojson_properties_not_null FROM api.logs_geojson_view ORDER BY name ASC;

\echo 'moorages geojson view'
--SELECT * FROM api.moorages_geojson_view;
SELECT id,name,geojson->'geometry' IS NOT NULL AS geojson_geometry_not_null, geojson->'properties' IS NOT NULL AS geojson_properties_not_null FROM api.moorages_geojson_view ORDER BY name ASC;
